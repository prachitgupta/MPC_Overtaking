...
class Draw_MPC_point_stabilization_v1(object):
    def __init__(self, robot_states: list, init_state: np.array, target_state: np.array, rob_diam=0.3,
        ...
        plt.grid('--')
        if export_fig:
            self.ani.save('./simpleKBM.gif', writer='imagemagick', fps=100)
        plt.show()
...
class Draw_MPC_Overtaking_Trajectory(object):
    def __init__(self, robot_states: list, human_states: list, robot_traj: list, human_traj: list, init_stateR: np.array, init_stateH: np.array, target_state: np.array, rob_diam=0.3, lead_diam=0.3, show_heatmap=True, export_fig=True):
        self.robot_states = robot_states
        self.human_states = human_states
        ...
        plt.grid('--')
        if export_fig:
            self.ani.save('./vnew.gif', writer='imagemagick', fps=100)
        plt.show()
        if show_heatmap:
            self.heatMap()
    ...
    def animation_init(self):
        ...
        self.target_circle = plt.Circle(self.target_state[:2], self.rob_radius, color='b', fill=False)
        self.ax.add_artist(self.target_circle)
        self.target_arr = mpatches.Arrow(self.target_state[0], self.target_state[1], self.rob_radius * np.cos(self.target_state[2]), self.rob_radius * np.sin(self.target_state[2]), width=0.2)
        self.ax.add_patch(self.target_arr)
        
        # Plot initial robot state and trajectory
        self.robot_body = plt.Circle(self.init_stateR[:2], self.rob_radius, color='r', fill=False)
        self.ax.add_artist(self.robot_body)
        self.robot_arr = mpatches.Arrow(self.init_stateR[0], self.init_stateR[1], self.rob_radius * np.cos(self.init_stateR[2]), self.rob_radius * np.sin(self.init_stateR[2]), width=0.2, color='r')
        self.ax.add_patch(self.robot_arr)
        self.robot_traj_line, = self.ax.plot([], [], color='r')

        # Plot initial human state and trajectory
        self.human_body = plt.Circle(self.init_stateH[:2], self.hum_radius, color='g', fill=False)
        self.ax.add_artist(self.human_body)
        self.human_arr = mpatches.Arrow(self.init_stateH[0], self.init_stateH[1], self.hum_radius * np.cos(self.init_stateH[2]), self.hum_radius * np.sin(self.init_stateH[2]), width=0.2, color='g')
        self.ax.add_patch(self.human_arr)
        self.human_traj_line, = self.ax.plot([], [], color='g')
        
        return self.target_circle, self.target_arr, self.robot_body, self.robot_arr, self.robot_traj_line, self.human_body, self.human_arr, self.human_traj_line
    ...
    def animation_loop(self, indx):
        ...
        self.robot_arr = mpatches.Arrow(robot_position[0], robot_position[1], self.rob_radius * np.cos(robot_orientation), self.rob_radius * np.sin(robot_orientation), width=0.2, color='r')
        self.ax.add_patch(self.robot_arr)
        self.robot_traj_line.set_data(self.robot_traj[:indx, 0], self.robot_traj[:indx, 1])

        # Update human position and trajectory
        human_position = self.human_states[indx][:2]
        human_orientation = self.human_states[indx][2]
        self.human_body.center = human_position
        self.human_arr.remove()
        self.human_arr = mpatches.Arrow(human_position[0], human_position[1], self.hum_radius * np.cos(human_orientation), self.hum_radius * np.sin(human_orientation), width=0.2, color='g')
        self.ax.add_patch(self.human_arr)
        self.human_traj_line.set_data(self.human_traj[:indx, 0], self.human_traj[:indx, 1])
        
        return self.robot_arr, self.robot_body, self.robot_traj_line, self.human_arr, self.human_body, self.human_traj_line