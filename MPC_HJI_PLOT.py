...
if __name__ == '__main__':
    ...
    human = np.array(h)
    ##save trajectories
    np.savez('trajectories.npz', robot = robot, human = human)
    ...
    yaw_c = []
    for array in xx:
        x_coordinates.append(array[0][0])  # Extract x coordinate (state 1)
        y_coordinates.append(array[1][0])  # Extract y coordinate (state 2)
        v.append(array[3][0])
        yaw_c.append(array[2][0])
    #robot_vel = xx[:][3]
    # Create the first subplot for robot position
    plt.subplot(3, 1, 1)
    plt.plot( x_coordinates ,y_coordinates, "--", color="red")  # Dashed line at 5
    plt.xlim(-30, 25)
    plt.ylim(0, 5)
    plt.ylabel("ROBOT position y(m)")
    plt.xlabel("robot pose x")

    plt.subplot(3, 1, 2)
    plt.plot(t_c, v, "-.", color="red", linewidth=0.8)  # Your reference y
    plt.xlim(0,10)
    plt.ylim(-5, 15)
    plt.ylabel("velocity")
    plt.xlabel("Time")
    ...
    plt.subplot(3, 1, 3)
    plt.plot(t_c, yaw_c, "-.", color="red", linewidth=0.8)  # Your reference y
    plt.xlim(0,10)
    plt.ylim(-np.pi, np.pi)
    plt.ylabel("yaw")
    plt.xlabel("Time")
    plt.tight_layout()  # Automatically adjust subplot parameters to give specified padding
    ## visualise value function 
